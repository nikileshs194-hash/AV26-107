...
export interface CycloneAlertData {
  type:        'cyclone_alert';
  probability: number;
  risk_level:  string;
  category:    string;
...
interface Props {
  visible:   boolean;
  data:      CycloneAlertData | null;
  onDismiss: () => void;
}

export default function CycloneAlertModal({ visible, data, onDismiss }: Props) {
  ...
  const category = data.category ?? 'Cyclonic Storm';
  const risk     = data.risk_level ?? 'High';

  const openSafetyInfo = () => {
    Linking.openURL(
      `https://www.google.com/maps/search/cyclone+shelter/@${data.user_lat},${data.user_lon},13z`
    );
  };
  ...
  const handleSOS = async () => {
    try {
      ...
      await sendSOS(
        ...
        data.user_lon,
        undefined,
        'Cyclone Emergency — Seeking Shelter',
      );
      setSosSent(true);
  ...
  const handleSafe = () => {
    ...
    Alert.alert(
      '✅ Marked Safe',
      'Stay indoors, away from windows. Monitor IMD bulletins for updates.',
      ...
      <View style={styles.overlay}>
        <Animated.View style={[styles.sheet, { transform: [{ translateY: slideAnim }] }]}>
          ...
          <ScrollView contentContainerStyle={styles.content} bounces={false} showsVerticalScrollIndicator={false}>
            ...
            {/* Titles */}
            <Text style={styles.title}>CYCLONE ALERT</Text>
            <Text style={styles.subtitle}>
              {risk} cyclone risk ({pct}%) detected{'\n'}near your current area.
            </Text>
            <Text style={styles.action}>Seek shelter immediately.{'\n'}Stay indoors, away from windows.</Text>
            ...
            <View style={styles.row}>
              <View style={[styles.rowIcon, { backgroundColor: '#f5f3ff' }]}>
                <Ionicons name="alert-circle-outline" size={22} color="#7c3aed" />
              </View>
              <View style={styles.rowText}>
                <Text style={styles.rowLabel}>IMD CATEGORY</Text>
            ...
            <View style={styles.row}>
              <View style={[styles.rowIcon, { backgroundColor: '#fff7ed' }]}>
                <Ionicons name="thermometer-outline" size={22} color="#ea580c" />
              </View>
              <View style={styles.rowText}>
            ...
            <View style={styles.row}>
              ...
              <View style={styles.rowText}>
                <Text style={styles.rowLabel}>FIND CYCLONE SHELTER</Text>
            ...
            <View style={styles.tipsBox}>
              ...
              <Text style={styles.tipsText}>• Stay indoors, away from windows and doors{'\n'}• Avoid flooded roads and coastal areas{'\n'}• Keep emergency kit ready (water, food, torch){'\n'}• Follow IMD / local authority advisories</Text>
            </View>
...
const styles = StyleSheet.create({
  ...
  title: {
    ...
    color: '#7c3aed', letterSpacing: 0.5, textAlign: 'center', marginBottom: 10,
  },
  subtitle: {
  ...
  action: {
    ...
    color: '#7c3aed', textAlign: 'center', lineHeight: 26, marginBottom: 18,
  },
  ...
  tipsBox:   { width: '100%', backgroundColor: '#faf5ff', borderRadius: 14, borderWidth: 1, borderColor: '#ddd6fe', padding: 14, marginBottom: 20 },
  tipsTitle: { fontFamily: 'PlusJakartaSans_700Bold', fontSize: 12, color: '#7c3aed', letterSpacing: 0.5, marginBottom: 8 },
  tipsText:  { fontFamily: 'PlusJakartaSans_400Regular', fontSize: 13, color: '#4c1d95', lineHeight: 22 },
